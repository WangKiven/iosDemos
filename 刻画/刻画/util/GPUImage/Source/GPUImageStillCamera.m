...
- (void)capturePhotoAsPNGUIImageProcessedUpToFilter:(GPUImageOutput<GPUImageInput> *)finalFilterInChain withCompletionHandler:(void (^)(UIImage *processedPNG, NSError *error))block
{
    
    
    [self capturePhotoProcessedUpToFilter:finalFilterInChain withImageOnGPUHandler:^(NSError *error) {
        UIImage *pngImage = nil;
        
        if(!error){
            @autoreleasepool {
                
                pngImage = [finalFilterInChain imageFromCurrentFramebuffer];
